...
export class Pst_SceneBoss
{
    ...
    async init()
    {
        ...
        const pacmanShader = device.createShaderModule({
            code: `struct U { model: mat4x4f, } @group(0) @binding(0) var<uniform> u: U;
                   ...
                   @fragment fn fs() -> @location(0) vec4f { return vec4f(1.0, 1.0, 0.0, 1.0); }`
        });
        ...
        const cherryShader = device.createShaderModule({
            code: `struct U { model: mat4x4f, } @group(0) @binding(0) var<uniform> u: U;
                   @vertex fn vs(@location(0) p: vec3f) -> @builtin(position) vec4f { return u.model * vec4f(p, 1.0); }
                   @fragment fn fs() -> @location(0) vec4f { return vec4f(1.0, 0.5, 1.0, 1.0); }`
        });
        const laserShader = device.createShaderModule({
            code: `
                struct U { model: mat4x4f, }
        ...
        this.skullPipeline = skullProto.pipeline;
    ...
    loop()
    {
        ...
        if (kb.isDown('ArrowRight')) { this.pacman.tileX += 5 * dt; this.pacman.dirX = 1; this.pacman.dirY = 0; }
        if (kb.isDown('ArrowLeft'))  { this.pacman.tileX -= 5 * dt; this.pacman.dirX = -1; this.pacman.dirY = 0; }
        ...
        if (px < 0.3) this.pacman.tileX = 0.3;
        if (px > this.worldW - 0.3) this.pacman.tileX = this.worldW - 0.3;
        if (py < 0.3) this.pacman.tileY = 0.3;
        if (py > this.worldH - 0.3) this.pacman.tileY = this.worldH - 0.3;
        ...
        {
            this.skullTimer = 1.5;
            this.skulls.push(new Pst_Scull(this.boss.x, this.boss.y, this.boss.angle));
        }
        for (const s of this.skulls) s.update(dt);
        this.skulls = this.skulls.filter(s => s.alive);
        for (const s of this.skulls)
        {
            if (Math.hypot(px - s.x, py - s.y) < 0.5) this.gameOver = true;
        }
        ...
        if (this.canAttack)
        {
            if (Math.hypot(px - this.boss.x, py - this.boss.y) < 2.5)
            {
                this.bossHp--;
                this.canAttack = false;
                if (this.bossHp <= 0) this.victory = true;
            }
        }
        ...
        for (const c of this.cherries)
        {
            ...
            model[12] = c.x * scaleX - 1; model[13] = c.y * scaleY - 1;
            device.queue.writeBuffer(this.cherryUB, 0, model);
            pass.setPipeline(this.cherryPipeline);
            pass.setBindGroup(0, this.cherryBG);
            const prim = Pst_Primitive.circle(0.2, 12, 0, 0);
            let vs = prim.vertices.byteLength; if (vs % 4) vs += 4 - vs % 4;
            let is = prim.indices.byteLength; if (is % 4) is += 4 - is % 4;
            ...
            pass.setVertexBuffer(0, vbo); pass.setIndexBuffer(ibo, 'uint16');
            pass.drawIndexed(prim.indices.length);
        }

        model.fill(0);
        model[0] = scaleX; model[5] = scaleY; model[10] = 1; model[15] = 1;
        model[12] = px * scaleX - 1; model[13] = py * scaleY - 1;
        ...
        for (const s of this.skulls)
        {
            s.draw(device, pass, this.skullPipeline, this.skullBG, this.skullUB, scaleX, scaleY);
        }
